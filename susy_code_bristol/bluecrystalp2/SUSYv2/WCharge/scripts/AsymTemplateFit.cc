...
void AsymTemplateFit()
{
  ...
  int ieff=80; float lumi=869.; int METCut=0; int reco = 0;bool corr=false ;bool DATA=true;
  ...
  for (int ib=0; ib<22;ib++){
    ...
    //TODO move NTP variables outside of eta charge loop and in to its own place
    //have 4 variables, con/sig for barrel/endcap for + and -
    int i_ECAL; //what ecal bin we are in
    // TString ecalbin[4]={"_EB_pos","_EB_neg","_EE_pos","_EE_neg"};
    if (ib<14){
      i_ECAL = 0;
    }
